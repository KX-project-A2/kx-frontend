...
import { CategoryPresetModal } from '@/components/domain/home/CategoryPresetModal';
...
/** 홈 화면 프롬프트 제안 칩 -> presets_catalog.json category 매핑 */
const SUGGESTION_CATEGORY_MAP: Record<string, string> = {
  '사이버펑크 도시의 밤': 'Cyberpunk',
  '미니멀 제품 광고컷': 'Product Ad Shot',
  // '수채화 스타일 고양이' / '판타지 캐릭터 시트' / '시네마틱 풍경 영상'
  // → presets_catalog.json(110건)에 매칭 카테고리 없어 칩 자체를 PROMPT_SUGGESTIONS에서 제거함 (mockData.ts 참고)
};
...
export default function Home() {
  ...
  return (
    <div className="mx-auto flex max-w-[1120px] flex-col gap-16 px-10 py-12">
      {/* ① hero + prompt */}
      <section className="relative flex flex-col items-center gap-6 overflow-hidden rounded-card pt-8 text-center">
        <video
          ...
          playsInline
        />
        <div className="absolute inset-0" style={{ background: 'rgba(11,9,18,0.55)' }} />

        <div className="relative z-10 flex flex-col items-center gap-6">
          <div className="flex items-center gap-2">
            <div
              className="flex items-center gap-2 rounded-chip px-3 h-7"
              style={{
            ...
            <div
              ...
              style={{
                background: 'rgba(234, 251, 47, 0.16)',
            ...
            >
              <Zap
                size={13}
                strokeWidth={2.5}
                style={{ color: 'var(--accent-500)', fill: 'var(--accent-500)' }}
              />
              <span className="text-label" style={{ color: 'var(--accent-500)' }}>
                매일 무료 크레딧 +50
              </span>
            </div>
          </div>
            ...
            <div className="mt-1 flex flex-wrap gap-2 px-1">
              {PROMPT_SUGGESTIONS.map((s) => (
                <Chip
                  ...
                  onClick={() => {
                    setPrompt(s);
                    const category = SUGGESTION_CATEGORY_MAP[s];
      ...
      <section className="flex flex-col gap-6">
        ...
        <Panel level={1} bordered className="relative overflow-hidden">
          ...
          <div
            ...
            style={{ background: 'rgba(11,9,18,0.35)' }}
          >
            <button
